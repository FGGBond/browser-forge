...
describe('recording workspace shell', () => {
  ...
  it('defines a stable 240px navigation, 68px rail, and 476px context contract', () => {
    ...
    expect(css).toMatch(/\.app-shell\.sidebar-collapsed \{[^}]*grid-template-columns:\s*var\(--sidebar-rail-w\) minmax\(560px,1fr\) var\(--context-w\)/s)
    expect(css).toMatch(/--context-w:\s*476px/)
    expect(css).toMatch(/@media \(max-width:\s*1275px\)[\s\S]*?grid-template-columns:\s*var\(--sidebar-rail-w\) minmax\(560px,1fr\) var\(--context-w\)/)
    expect(css).toMatch(/@media \(max-width:\s*1103px\)[\s\S]*?\.workspace-context-host[^}]*position:\s*fixed/)
  ...
  it('defines an interruptible compositor-only sidebar motion contract', () => {
    ...
    expect(css).toMatch(/--sidebar-label-enter-duration:\s*170ms/)
    expect(css).toMatch(/--sidebar-label-exit-duration:\s*110ms/)
  ...
  it('stores shell collapse state separately from recording data', () => {
    ...
    expect(app).toContain('data-context-host')
    expect(app).toContain('createContextPane')
    expect(app).toContain('syncWorkspaceContext')
    ...
    expect(detail).not.toContain('renderPromptEditor')
    expect(detail).not.toContain('data-analysis-pane')
