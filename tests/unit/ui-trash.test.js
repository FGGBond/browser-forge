...
describe('App recycle bin UI', () => {
  it('uses a single management list with poster, metadata, and direct row actions', async () => {
    ...
    const restore = row.getByRole('button', { name: '恢复录制' })
    const remove = row.getByRole('button', { name: '永久删除' })
    await restore.waitFor()
    await remove.waitFor()
    expect((await restore.getAttribute('class')).split(/\s+/)).toContain('primary')
    expect((await remove.getAttribute('class')).split(/\s+/)).not.toContain('primary')
    expect((await remove.getAttribute('class')).split(/\s+/)).toEqual(expect.arrayContaining(['danger', 'quiet']))
    const density = await row.evaluate(element => {
      const box = element.getBoundingClientRect()
      const poster = element.querySelector('.trash-poster').getBoundingClientRect()
      const style = getComputedStyle(element)
      return { height: box.height, posterWidth: poster.width, radius: parseFloat(style.borderTopLeftRadius), shadow: style.boxShadow }
    })
    expect(density.height).toBeLessThanOrEqual(124)
    expect(density.posterWidth).toBeLessThanOrEqual(136)
    expect(density.radius).toBeLessThanOrEqual(12)
    expect(density.shadow).toBe('none')
